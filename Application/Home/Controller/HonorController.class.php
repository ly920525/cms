...
class HonorController extends CommonController{
    ...
    public function honor(){
        ...
        if($_GET['issuer']){
            $hc = M('honor_content');
            $count = $hc->where('pid='.$issuer)->count();//查询满足条件总记录数
            //var_dump($count);die;
            ...
            $lists = $hc->where('pid='.$issuer)->order('time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
        }else{
            $hc = M('honor_content');
            $count = $hc->where('pid='.$issuer)->count();//查询满足条件总记录数
            $Page  = new \Think\Page($count,6);// 实例化分页类 传入总记录数和每页显示的记录数()
